...
def main():
  ...
  for f in all_files:
    global_all_done[all_strip(f, ["followers_", "friends_", ".txt"])] = True
  ...
  if(resume):
    ...
    cur_files = [f for f in listdir(path + str(start_level-1)) if isfile(path + str(start_level-1) + "/" + f)]  
  ...
  while(not file_queue.empty()):
    ...
    # print("Processing: " + f + " at level " + str(tmp_level))
    ...
    with open(file_name, mode='r') as inptr:
      ...
      while True:        
        try:
          # We already have followers in global repository
          
          ############################################################
          #### EXPENSIVE OPERATIONS THAT CAN BE AVOIDED IF NEEDED ####
...
def generateFollowers(org, level, thread_num, log_file_writer, follower_count_writer, lock):
  try:
    ...
    outptr.close()
    shutil.copy(path + str(level) + "/followers_" + org + ".txt", global_repository)
...
if __name__=="__main__":
  ...
  link = "https://mobile.twitter.com/nytimes/following"
  req = Request(link, headers = headers)
  page = urlopen(req)
  printPage(page, "Test")
